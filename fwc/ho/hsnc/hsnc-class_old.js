...
class HsncManager {
  ...
  static load() {
    ...
    for (let i = 0; i < hsncItem.length; i++) {
      hsncItem[i].isEnabled = false;
    }
    /* !!!!! ;v1.1.22-a.4; !!!!! */
  }
  static resizeDisplay() {
  ...
  static loadEvent() {
    ...
    for (let i = 0; i < hsncItemContainer.length; i++) {
      hsncItemContainer[i].addEventListener("click", HsncHandler.hsncItemContainer);
    }
  }
  /* !!!!! ;v1.1.22-a.4; !!!!! */
...
class HsncHandler {
  static hsncItemContainer(eventData) {
    const currentTarget = eventData.currentTarget;
    const closest = currentTarget.closest(".hsnc-item");
    const hsncItem = closest;
    const hsncSubList = closest.querySelector(".hsnc-sub-list");
    const hsncSubItem = closest.querySelectorAll(".hsnc-sub-item");
    /*  */
    const hsncSubItemText = hsncItem.querySelectorAll(".hsnc-sub-item-text");
    const hsncSubItemBr = hsncItem.querySelectorAll(".hsnc-sub-item-br");
    /*  */
    const clDataEnabled = "cl-hsnc-item-container-enabled";
    let shouldEnable = false;
    let clType = "remove";
    if (!hsncItem.isEnabled) {
      shouldEnable = true;
      clType = "add";
    }
    if (shouldEnable) {
      hsncSubList.style.height = (hsncSubItem.length * 4).toString() + "rem";
      for (let i = 0; i < hsncSubItemText.length; i++) {
        const width = hsncSubItemText[i].offsetWidth;
        hsncSubItemBr[i].style.width = width.toString() + "px";
      }
    } else {
      hsncSubList.style.height = "";
    }
    hsncSubList.classList[clType](clDataEnabled);
    hsncItem.isEnabled = shouldEnable;
  }
  /* ================================================== */
